...
skylineLoader.load("MochaSteve256-2023.stl", function (geometry) {
  ...
  setRotation(mesh, -0.5 * Math.PI, 0, -0.5 * Math.PI);
  mesh.position.x = 2;
  mesh.position.z = -2;
  scene.add(mesh);
});
...
function moveCamera(): void {
  ...
  if (delta < 0 && Math.abs(delta) > smoothDeltaThreshold) {
    tV += acceleration;
    if (tV < 0)
      if (Math.abs(tV) > smoothVelocityThreshold) tV -= acceleration;
      else tV += acceleration / 5;
